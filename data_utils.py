...
class ABSADataset(Dataset):
    def __init__(self, datafile, tokenizer, opt, deptype2id = None, postype2id = None,dep_order = "second"):
        self.datafile = datafile
        self.depfile = "{}.dep".format(datafile)
        self.posfile = "{}.pos".format(datafile)
        ...
        self.posinfo = ABSADataset.load_posfile(self.posfile)
        self.polarity2id = self.get_polarity2id()
        self.feature = []
        for sentence,depinfo,posinfo in zip(self.textdata, self.depinfo, self.posinfo):
            self.feature.append(self.create_feature(sentence, depinfo, posinfo))
    ...
    def create_feature(self, sentence, depinfo, posinfo):
        ...
        pos_ids = []
        for item in pos:
            pos_ids.append(self.postype2id[item['pos']])
        ...
        left_len_list = []
        for i in left_valid_ids:
            if i == 1:
                left_len_list.append(1)
        left_len = len(left_len_list)
        aspect_len_list = []
        for i in aspect_valid_ids:
            if i == 1:
                aspect_len_list.append(1)
        aspect_len = len(aspect_len_list)

        input_ids = [cls_id] + left_token_ids + aspect_token_ids + right_token_ids + [sep_id] + aspect_token_ids + [sep_id]
        valid_ids = [0] + left_valid_ids + aspect_valid_ids + right_valid_ids + [0]
        mem_valid_ids = [0] * left_len + [1] * aspect_len
        ...
        final_dep_adj_matrix = [[0]*self.max_key_len for _ in range(self.max_key_len)]
        final_dep_value_matrix = [[0]*self.max_key_len for _ in range(self.max_key_len)]

        for i in range(len(token_head_list)):
             for j in range(len(dep_adj_matrix[i])):
                 if j >= self.max_key_len:
                     break
                 final_dep_adj_matrix[i][j] = dep_adj_matrix[i][j]
                 final_dep_value_matrix[i][j] = self.deptype2id[dep_type_matrix[i][j]]
        ...
        left_token_ids = self.tokenizer.id_to_sequence(left_token_ids)
        pos_ids = self.tokenizer.id_to_sequence(pos_ids)
        ...
        return {
            ...
            "left_token_ids": torch.tensor(left_len),
    ...
    def load_deptype_map(opt):
        ...
        deptype_map = {"none": 0, "self":1}
        for deptype in sorted(deptype_set, key=lambda x:x):
            deptype_map[deptype] = len(deptype_map)
        return deptype_map

    @staticmethod
    def load_postype_map(opt):
        postype_set = set()
        for filename in [opt.train_file, opt.test_file,opt.val_file]:
            filename = "{}.pos".format(filename)
            if os.path.exists(filename) is False:
                continue
            data = ABSADataset.load_posfile(filename)
            for pos_info in data:
                for item in pos_info:
                    postype_set.add(item['pos'])
        postype_map = {"none": 0}
        for postype in sorted(postype_set,key=lambda x:x):
            postype_map[postype] = len(postype_map)
        return postype_map
